...
** SQLite Read-Eval-Printing-Loop in the cloud
   ...
   * You can now run SQLite commands and also access the ~bash~ shell.
     ...
     #+caption: My SQLite REPL dashboard
     #+attr_html: :width 500px
     [[./img/replsqlite.png]]
...
** NEXT
   ...
   * SELECT round-up continued - down the pipeline

* NEXT SELECT round-up - w8s14 - March 3
** Objectives

   * [ ] Review the entire SELECT pipeline (for SQLite)

** NEXT SELECT round-up
* References
