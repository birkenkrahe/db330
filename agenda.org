...
* Review test 1, SELECT - w6s9 - February 15, 2022
** News
...
** Objectives
   ...
   * [ ] Introduction to SELECT (DataCamp review)
...
** Test 1 results
   ...
   * Let's make some plots: histogram, boxplot and density plot. I'd
     ...
     #+begin_src R :exports both :session :results output graphics file :file ./img/test1hist.png
       hist(results, main="Histogram of test 1 results, CSC 420 Spring 2022")
     #+end_src
     ...
     #+begin_src R :exports both :session :results output graphics file :file ./img/test1box.png
       boxplot(results, main="Test 1 results, CSC 420 Spring 2022")
     #+end_src
     ...
     #+begin_src R :exports both :session :results output graphics file :file ./img/test1density.png
       ave <- mean(results)
       d <- density(results)
       plot(d, col="steelblue",main="Test 1 Results CSC 420 Spring 2022")
...
*** Database architecture layer and example applications
...
*** Customer control for different cloud architectures

    [[https://github.com/birkenkrahe/db330/blob/main/FAQ.org#control-question-test-1-on-cloud-services][See FAQ]].
...
*** In research communication, "limitations" means "bias".
...
** Introduction to SELECT
   ...
   * Introduction to the SELECT pipeline
   * Continue SQLite session from last time:
     - dump SQL data / stdout
     - alter a table
     - class assignment
